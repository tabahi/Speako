...
function process_command(event_node_text)   //green boxes call this function
{
    ...
    {
        console.log(event_node_text.slice(event_node_text.indexOf("URL=")+4));
        window.open(event_node_text.slice(event_node_text.indexOf("URL=")+4), '_blank');
    }
...
function flag_set(flag_val) //state change tracking function
{
    ...
    else if(flag_val==1)    //human speaking turn
    {
        ...
        if(voiced) SP.define_speech_recognition(human_speech_response, on_human_speech_response_end, true);
...
export function voice_enabled(isChecked=document.getElementById("voice_enable_check").isChecked)
{
    if(isChecked)
    {
        ...
        const SpeechRecognition =  (window.hasOwnProperty('webkitSpeechRecognition')) ? webkitSpeechRecognition: SpeechRecognition;
